...
async Task RunAccountAsync(AccountConfig account, RuntimeConfig runtimeConfig, CancellationToken ct)
{
    ...
    Utility.WriteLine(ConsoleColor.Yellow, $"[{label}] 保活任务启动：每 {runtimeConfig.KeepAliveSecondsMin}-{runtimeConfig.KeepAliveSecondsMax} 秒强制重连一次。");
    var keepAliveTasks = activeDesktops.Select(d => KeepAliveWorkerWithForcedReset(api, account, d, runtimeConfig.KeepAliveSecondsMin, runtimeConfig.KeepAliveSecondsMax, ct));
...
async Task KeepAliveWorkerWithForcedReset(CtYunApi api, AccountConfig account, Desktop desktop, int keepAliveSecondsMin, int keepAliveSecondsMax, CancellationToken globalToken)
{
    var label = AccountLabel(account);
    ...
    {
        var keepAliveSeconds = RandomNumberGenerator.GetInt32(keepAliveSecondsMin, keepAliveSecondsMax + 1);
        ...
        try
        {
            ...
            Utility.WriteLine(ConsoleColor.Green, $"[{label}][{desktop.DesktopCode}] 连接已就绪，本次保持 {keepAliveSeconds} 秒...");
...
RuntimeConfig LoadRuntimeConfig()
{
    ...
    var keepAliveMin = config.KeepAliveSecondsMin ?? Math.Max(10, config.KeepAliveSeconds);
    var keepAliveMax = config.KeepAliveSecondsMax ?? keepAliveMin;
    return new RuntimeConfig(config.Accounts, keepAliveMin, keepAliveMax, dataDir);
}
...
record RuntimeConfig(
    ...
    int KeepAliveSecondsMin,
    int KeepAliveSecondsMax,
    string DataDir);
